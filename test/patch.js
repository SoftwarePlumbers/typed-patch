...
describe("Patch", () => {
        ...
        it("can diff maps", () => {
            ...
            let patch = Patch.compare(a,b,{map: true});
            ...
            patch = Patch.compare(b,a,{map: true});
            logger.debug(patch);
            expect(patch.name).to.equal(Patch.Map);
            expect(patch.data.length).to.equal(1);
        ...
        it("can patch maps", () => {
            ...
            let patch2 = Patch.compare(b,a,{map:true});
        ...
        it("can patch an map of articles - adding an item in the middle", () => {
            ...
            let diff = Patch.compare(array1, array2, { map: true });
            logger.debug(diff.toString());
            let array3=diff.patch(array1, { collectionElementFactory: Article.fromJSON } );
            logger.debug(array3);
        ...
        it("can patch an map of articles - via JSON", () => {
            ...
            let diff1 = Patch.compare(array1, array2, { map: true });
            let json1 = JSON.stringify(diff1.toJSON());
            let obj = JSON.parse(json1);
            let diff2 = Patch.fromJSON(obj);
            let json2 = JSON.stringify(diff2.toJSON());
            expect(json1).to.equal(json2);
            let array3=diff2.patch(array1, { collectionElementFactory: Article.fromJSON });
